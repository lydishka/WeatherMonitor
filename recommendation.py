...
def get_activity_advice(temperature, weather_condition, wind_speed):
    ...
    if "дождь" in condition or "пасмурно" in condition:
    ...
    elif temperature > 20 and "облачность" in condition or "облачно" in condition:
...
def get_all_recommendations(weather_data):
    ...
    return {
        'clothing': get_clothing_advice(temp, condition),
        'activities': get_activity_advice(temp, condition, wind_speed),
        'warnings': get_health_warnings(temp, humidity)
    }